...
def call_llm_with_tools(messages, query_embedding=None):
    response = openai.ChatCompletion.create(
        ...
        messages=messages,
        functions=tools,
        function_call={"name": "search_documents_in_chromadb"} if query_embedding else "auto",  # Auto call when not specific
        tool_functions={"search_documents_in_chromadb": search_documents_in_chromadb(query_embedding)} if query_embedding else None
    )
...
if prompt := st.chat_input("What is up?"):
    ...
    assistant_message = call_llm_with_tools(st.session_state.messages, query_embedding)
